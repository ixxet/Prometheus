...
| `cilium-l2-policy.yaml` | Tells Cilium to answer ARP for external and `LoadBalancer` IPs on `enp5s0`. | NIC-specific. If the physical NIC name changes, this must change too. | It does not expose any service until a `LoadBalancer` service exists. |
