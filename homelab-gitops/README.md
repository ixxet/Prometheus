...
- It does not yet include vLLM, ComfyUI, media, Immich, or Tailscale manifests.
...
| `apps/ai/ollama/` | Ollama deployment, model PVC, and fixed-IP `LoadBalancer` service. | Must respect single-GPU scheduling and model sizing for the RTX 3090. | It does not pull models automatically yet. |
| `apps/ai/open-webui/` | Open WebUI frontend wired to the in-cluster Ollama service. | Depends on storage and the Ollama backend. | It does not manage auth bootstrap or external identity. |
...
| `clusters/talos-tower/apps.yaml` | Reconcile AI applications only after infrastructure and storage are ready. | Currently suspended on purpose. | It must not bypass future SOPS secret handling. |
...
5. Add encrypted secret handling before vLLM, Immich, or anything token-bearing lands here.
6. Choose the DNS cutover window, then unsuspend `infra-dns`.
7. Unsuspend `apps-ai` after storage and DNS are ready.
